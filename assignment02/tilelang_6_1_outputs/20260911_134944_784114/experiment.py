...
The API was checked against upstream v0.1.7 and main. This script still needs
execution in the course's Linux/TileLang/CUDA environment for validation.
"""
...
def compile_cuda(nvcc, arch, directory):
    # Explicit NVCC architecture also avoids older target-string parsing bugs.
...
def main():
    ...
    for arch in ("sm_90a", "sm_100a"):
        ...
        status = {"target": f"cuda -arch={arch}", "lowering_ok": False, "nvcc_ok": False}
        metadata["targets"][arch] = status
        try:
            prim_func = make_gemm(T)
            (directory / "input_ir.txt").write_text(prim_func.script(), encoding="utf-8")
            with tvm.transform.PassContext(opt_level=3, instruments=[DumpPasses(passes)]):
                artifact = lower(
                    prim_func,
